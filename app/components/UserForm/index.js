...
const UserLabel = () => <span>{Icon({type: iconTypes.ACCOUNT})} {t('userId')}</span>
...
const UserForm = ({user, handleSubmit}) =>
  <form className='user-form' key={user.id} onSubmit={handleSubmit(submit)}>
    <FormGroup className='update-employee-group'>
      ...
      <Row>
        <Col sm={1}>
          <Button id='update-user' color='primary' type='submit'>
            {t('update')}
          </Button>
        </Col>
        <Col sm={1}>
          <Button id='cancel-update-user' color='danger' onClick={onCancel}>
            {Icon({type: iconTypes.CLOSE})} {t('cancel')}
          </Button>
        </Col>
      </Row>
    </FormGroup>
